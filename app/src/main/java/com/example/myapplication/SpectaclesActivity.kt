...
class SpectaclesActivity : AppCompatActivity(), ShirtAdapter.AddToCartClickListener, ShirtAdapter.OrderClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        supportActionBar?.title = "Spectacles"
        fetchProfileImage()

        // Initialize the Firebase Realtime Database reference for "shirts" category
        database = FirebaseDatabase.getInstance().reference.child("shopping_items").child("spectacles")
            ...
            cartRef.orderByChild("name").equalTo(shirt.name).addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(dataSnapshot: DataSnapshot) {
                    if (dataSnapshot.exists()) {
                        ...
                        Toast.makeText(this@SpectaclesActivity, "Item already in cart!", Toast.LENGTH_SHORT).show()
                    } else {
                        // Item does not exist in the cart, add it
                        val cartItemRef = cartRef.push()
                        cartItemRef.setValue(shirt)
                            .addOnSuccessListener {
                                Toast.makeText(this@SpectaclesActivity, "Item added to cart!", Toast.LENGTH_SHORT).show()
                            }
                            .addOnFailureListener {
                                Toast.makeText(this@SpectaclesActivity, "Failed to add item to cart.", Toast.LENGTH_SHORT).show()
                            }
                    }
                }
