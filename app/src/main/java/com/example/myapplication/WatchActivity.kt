...
class WatchActivity : AppCompatActivity(), ShirtAdapter.AddToCartClickListener, ShirtAdapter.OrderClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        database = FirebaseDatabase.getInstance().reference.child("shopping_items").child("watch")
            ...
            cartRef.orderByChild("name").equalTo(shirt.name).addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(dataSnapshot: DataSnapshot) {
                    if (dataSnapshot.exists()) {
                        ...
                        Toast.makeText(this@WatchActivity, "Item already in cart!", Toast.LENGTH_SHORT).show()
                    } else {
                        // Item does not exist in the cart, add it
                        val cartItemRef = cartRef.push()
                        cartItemRef.setValue(shirt)
                            .addOnSuccessListener {
                                Toast.makeText(this@WatchActivity, "Item added to cart!", Toast.LENGTH_SHORT).show()
                            }
                            .addOnFailureListener {
                                Toast.makeText(this@WatchActivity, "Failed to add item to cart.", Toast.LENGTH_SHORT).show()
                            }
                    }
                }
